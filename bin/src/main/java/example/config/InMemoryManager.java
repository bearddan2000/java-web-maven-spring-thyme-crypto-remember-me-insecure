...
public class InMemoryManager implements UserDetailsService {
    ...
    public ArrayList<example.model.User> findAll(){
        ArrayList<example.model.User> values = new ArrayList<example.model.User>(users.values());
        for (example.model.User u : values) {
          u.setPassword("");
          u.setChallenge(null);
        }
        return values;
